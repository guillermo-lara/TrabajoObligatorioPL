...
prg returns [String code]
: 'program' ID ';' blq '.' { $code = "#include <stdio.h>\n" +"void main() {\n"+ $blq.bloq+ "\n}"; }
| 'unit' ID ';' dcllist '.'{$code = "//Libreria"+ $ID.text + "\n";};

blq returns [String bloq]
: dcllist 'begin' sentlist 'end' {$bloq = $dcllist.dcllis +  $sentlist.senlis;};

dcllist returns [String dcllis]
: dcl dcllist {$dcllis = $dcl.dcll +$dcllist.dcllis;}| {$dcllis = "";} ;

sentlist returns [String senlis]
: sent  sentlistp {$senlis = $sent.sen +$sentlistp.senlisp;};

sentlistp returns [String senlisp]
: sent sentlistp {$senlisp = $sent.sen +$sentlistp.senlisp;}| {$senlisp = "";};
...
: 'CONST'
|'const' ctelist {$defconst = "#define " +$ctelist.ctelis;};

ctelist returns [String ctelis]
: ID '=' simpvalue ';' ctelistp {$ctelis = $ID.text +" " +$simpvalue.simp +"\n" +"#define " +$ctelistp.ctelisp;}; //$ctelis = $ID.text +" = " +$simpvalue.simp +" ; " +$ctelistp.ctelisp;

ctelistp returns [String ctelisp]
: ID '=' simpvalue ';' ctelistp {$ctelisp = $ID.text +" "+ $simpvalue.simp +"\n" +"#define " +$ctelistp.ctelisp;}
| {$ctelisp = "";} ;
...
: 'VAR'
|'var' defvarlist ';' {$defvari = $defvarlist.defvarlis;};

defvarlist returns [String defvarlis]
: varlist ':' tbas defvarlistp {$defvarlis = $tbas.vlex + $varlist.varlis +$defvarlistp.defvarlisp;}; //$defvarlis = $varlist.varlis +' : ' +$tbas.vlex +$defvarlistp.defvarlisp; LUEGO PASO A $defvarlis = $tbas.vlex + $varlist.varlis ;

defvarlistp returns [String defvarlisp]
: ';' varlist ':' tbas defvarlistp {$defvarlisp = ";" +"\n" +$tbas.vlex +$varlist.varlis +$defvarlistp.defvarlisp;} | {$defvarlisp= "";}; //$defvarlisp = "; " +$varlist.varlis +" : " +$tbas.vlex +$defvarlistp.defvarlisp;
...
: 'PROCEDURE'
|'procedure' ID formal_paramlist ';' blq ';' {$defproceso ="void " +$ID.text +" " +$formal_paramlist.for_paramli +"\n" +"{" +"\n" +$blq.bloq +"\n" +"}";}; // añadido despues +\n +"{" +\n +$blq.bloq +\n +"}"

deffun returns [String deffuncion]
: 'FUNCTION'
|'function' ID formal_paramlist ':' tbas ';' blq ';' {$deffuncion =$tbas.vlex +$ID.text +" " +$formal_paramlist.for_paramli +"\n" +"{" +"\n" +$blq.bloq +"\n" +"}";};//f = new Function($ID.text, $tbas.vlex...) creo que el tbas no tendria nada que ver  // añadido despues +\n +"{" +\n +$blq.bloq +\n +"}"
...
: varlist ':' tbas formalparamaux {$for_para = $varlist.varlis +" :";};
...
: oplog expcond expcondp {$expconp = $oplog.oplo +" " +$expcond.expcon +" " +$expcondp.expconp;}|{$expconp ="";} ;
...
| '(' expcond ')' {$factorcon = "( " +$expcond.expcon +" )" ;}| '!' factorcond {$factorcon = "not " +$factorcond.factorcon ;};
